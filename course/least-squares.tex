...
\chapter{Galerkin weighted residual method}

Ground truth:
$$
\varphi := \frac{1}{6} x^3 + \frac{1}{2} x^2 + \frac{1}{3} x
$$
...
\left\{
\begin{split}
\frac{d^2}{dx^2}\varphi = x+1, \quad 0 < x < 1\\
...
$$
\int\limits_0^1 w_i \cdot \left(\frac{d^2\tilde{\varphi}}{dx^2} - (x+1)\right) dx = 0, \quad i = 0\dots 3
$$

Note that $w_i(x)$ is equal to zero outside the interval $[x_{i-1}, x_{i+1}]$, this allows us to rewrite the system:
$$
\int\limits_{x_{i-1}}^{x_{i+1}} w_i \cdot \frac{d^2\tilde{\varphi}}{dx^2}\,dx - \int\limits_{x_{i-1}}^{x_{i+1}} w_i\cdot (x+1)\,dx = 0, \quad i = 0\dots 3
$$
...
\int\limits_{x_{i-1}}^{x_{i+1}} \frac{dw_i}{dx} \cdot \frac{d\tilde{\varphi}}{dx}\,dx + \int\limits_{x_{i-1}}^{x_{i+1}} w_i\cdot(x+1)\,dx = 0, \quad i = 0\dots 3
$$
