...
&= \frac{\partial}{\partial x_i}  \Biggl( 
...
\underbrace{a_{ii}x_i^2}_{k_1 = i, k_2 = i}\Biggr) = \\ \pause
...
Imagine a car going at $v_0=0.5$ m/s. The goal is to accelerate to $v_n=2.3$ m/s in $n=30$ s maximum.
We can control the acceleration $u_i$ via the gas pedal:
$$
v_{i+1} = v_i + u_i
$$
...
$$
\min J(\vec{v},\vec{u}) \quad s.t.\quad  v_{i+1} = v_i + u_i = v_0 + \sum\limits_{j=0}^{i-1}u_j \quad \forall i \in 0..n-1
$$
...
$$
J(\vec{v}, \vec{u}) := \sum\limits_{i=1}^n \left(\sum\limits_{j=0}^{i-1}u_j-v_n+v_0\right)^2
$$
\end{minipage}
\qquad
\begin{minipage}{.45\linewidth}
...
\pause
\vspace{10pt}
\only<2>{\inputminted[fontsize=\footnotesize,frame=single,linenos=true]{python}{listings/example_6.1_a.py}}
...
$
J(\vec{v},\vec{u}) := \sum\limits_{i=1}^n (v_i-v_n)^2 + \highlight{4}\sum\limits_{i=0}^{n-1} u_i^2 = \sum\limits_{i=1}^n \left(\sum\limits_{j=0}^{i-1}u_j-v_n+v_0\right)^2 + \highlight{4}\sum\limits_{i=0}^{n-1} u_i^2
$
...
\end{minipage}
\begin{minipage}{.45\linewidth}
\quad\includegraphics[width=\linewidth]{../manuscript/img/pie-1d.png}
\end{minipage}
\pause
\begin{minipage}{.35\linewidth}
Least squares formulation:
$$
\min\limits_{f} \int_0^{2\pi} \|f' - g'\|^2
$$
with $f(0)=1,~f(2\pi) = 3$
\end{minipage}
\pause
...
we compute a $w \times h$ pixels image $f$, solve in the least squares sense:
$$
